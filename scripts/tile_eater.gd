...
static func initialize_dirt_border_for_tilemap(target_tilemap: TileMap) -> void:
	if target_tilemap == null:
		return
	if target_tilemap.has_meta(DIRT_BORDER_META_KEY):
		return
	var target_dirt_border_layer = target_tilemap.get_node_or_null(DIRT_BORDER_LAYER_NAME) as TileMapLayer
	if target_dirt_border_layer == null:
		return
	target_dirt_border_layer.clear()
	var floor_cells: Array[Vector2i] = []
	for cell in target_tilemap.get_used_cells(0):
		if _is_walkable_cell_in_tilemap(target_tilemap, cell):
			floor_cells.append(cell)
	if not floor_cells.is_empty():
		target_dirt_border_layer.set_cells_terrain_connect(
			floor_cells,
			DIRT_BORDER_TERRAIN_SET,
			DIRT_BORDER_TERRAIN,
			false
		)
	target_tilemap.set_meta(DIRT_BORDER_META_KEY, true)


static func _is_walkable_cell_in_tilemap(target_tilemap: TileMap, cell: Vector2i) -> bool:
	if target_tilemap == null:
		return false
