...
const GHOST_COOLDOWN_TINT := Color(1.0, 0.2, 0.2, 1.0)
...
func update_ghost_state(delta: float) -> void:
	...
	update_ghost_possession_cooldown(delta)
...
func start_ghost_possession(target: Node2D, duration: float) -> void:
	...
	visuals.modulate.a = 0.0
	ghost_offscreen_time = 0.0
	ghost_possession_cooldown_left = 0.0
	update_ghost_cooldown_visual()
...
func start_ghost_cooldown() -> void:
	ghost_possession_cooldown_left = GHOST_POSSESSION_COOLDOWN
	update_ghost_cooldown_visual()


func update_ghost_possession_cooldown(delta: float) -> void:
	if ghost_possession_cooldown_left <= 0.0:
		return
...
func update_ghost_cooldown_visual() -> void:
	if ghost_color == null:
		return
	if ghost_possession_cooldown_left > 0.0:
		ghost_color.color = GHOST_COOLDOWN_TINT
