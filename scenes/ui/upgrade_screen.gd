...
func get_display_name(upgrade: AbilityUpgrade, current_upgrades: Dictionary) -> String:
	...
	if upgrade.id == "poison_spit":
		return "Poison Spit Level 1"
	if upgrade.id in ["sword_level", "axe_level", "fireball_level", "boomerang_level", "dig_level", "whip_level", "poison_spit_level"]:
